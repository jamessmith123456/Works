...
class Application(Frame):  # 从Frame派生出Application类，它是所有widget的父容器
    ...
    def createWidgets(self):
        self.helloLabel = Label(self, text='建筑装修问答系统').grid(row=0,column=7)  # 创建一个标签显示内容到窗口
        self.blankLabel1 = Label(self, text='').grid(row=2, column=7)

        self.question_mean = Label(self, text='输入问题:').grid(row=5,sticky=E)
        self.input_question = Entry(self)  # 创建一个输入框，以输入均值
        self.input_question.grid(row=5,column=1)
