...
class QuestionTemplate():
    ...
    def get_book_author(self):
        ...
        cql = f"match (m:Movie)-[]->() where m.title='{book_name}' return m.rating"
        print(cql)
        answer = self.graph.run(cql)[0]
        print(answer)
        final_answer=book_name+"书籍作者为"+str(answer)+"！"
        return final_answer
    # 1:nm 上映时间
    ...
    def get_actor_birthday(self):
        ...
        #final_answer = graph.run("MATCH  (n: Architecture{question: '内平开下悬窗适用于哪些窗型？' }) return  n.answer;")