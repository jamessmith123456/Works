...
df1 = df['dx'][0:2160].values#人流数据  (2160,)


tem1 = data['1T'][0:2160].values #(2160,)
tem2 = data['2T'][0:2160].values #(2160,)
tem3 = data['3T'][0:2160].values #(2160,)
tem4 = data['4T'][0:2160].values #(2160,)



#巴特沃斯滤波
b,a = ss.butter(3,0.08,'low')  
t1 = ss.filtfilt(b,a,tem1)-4 #(2160,)
t2 = ss.filtfilt(b,a,tem2)-4 #(2160,)
t3 = ss.filtfilt(b,a,tem3)-4 #(2160,)
t4 = ss.filtfilt(b,a,tem4)-4 #(2160,)
df3 = ss.medfilt(df1,19) #(2160,)
t2 = t2-0.2

tem_ = np.vstack((t1,t2,t3,t4)) #(4,2160)
...
x = np.vstack((x1,x2,x3,x4)) #(4,2160)
y = y1 #(2160,)

X = x[:,0:1440].T #(1440,4)
T = y[0:1440].reshape((1440,1)) #(1440,1)

X2 = x[:,1440:2160].T #(720,4)
T2 = y[1440:2160].reshape((720,1)) #(720,1)
