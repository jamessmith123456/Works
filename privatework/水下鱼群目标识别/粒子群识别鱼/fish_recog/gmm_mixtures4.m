function [bestk,bestpp,bestmu,bestcov,dl,countf] = gmm_mixtures4(y,kmin,kmax,regularize,th,covoption)
%
% 
% Usage syntax:
...
for i=1:k
   semi_indic(i,:) = gmm_multinorm(y,estmu(:,i),estcov(:,:,i));
...
while(cont)    % this inner loop is the component-wise EM algorithm with the
   ...
   while comp <= k
            ...
            if estpp(comp)==0
             ...
             if (sum(sum(isnan(estmu)))>0) || (sum(sum(sum(isnan(estcov))))>0)
                 return;
             end
             
             if (covoption == 2)|(covoption == 3)
                for kk = 1:k
                   semi_indic(kk,:) = gmm_multinorm(y,estmu(:,kk),estcov(:,:,kk)); 
                end
             end
             ...
             if killed==0
                ...
                % indicator variables...
                semi_indic(comp,:) = gmm_multinorm(y,estmu(:,comp),estcov(:,:,comp)); 
