function stats = append_fastcoprops(varargin)
% Extended and faster version of GRAYCOPROPS - Lucia IPAB 090220
%
%
...
%
%   'Entropy'       measure of the disorder of an image. Emtropy is inversely 
%                   correlate to energy. Entropy is 0 for a constant image
%                   Range = [0 ?]
%                   
...
%                   a diagonal GLCM.
%
%   'InvDiffMoment' Inverse Difference Moment (sometime also called Homogeneity) 
%                   measure homogeneity. Inversely related to both contrast
%                   and energy
%  
%   'ClusterShade'
%
...
allStats = {'Contrast','Correlation','Energy','Entropy','Homogeneity','InvDiffMoment',...
    'ClusterShade','ClusterProminence','MaxProbability','Autocorrelation',...
...
for p = 1 : numGLCM
  ...
  m=0; % meanIndex not calculated
      
  % Calculate fields of output stats structure.
  for k = 1:numStats
      name = requestedStats{k};

      % calculate mr and mc only once
      if ~m
          switch name
              case {'ClusterShade','ClusterProminence'}
                  mr = meanIndex(r,tGLCM);
                  mc = meanIndex(c,tGLCM);
                  m=1; % meanIndex calculated
                  term1 = (r + c - mr - mc);
                  term2 = term1 .* term1;
              case 'Correlation'
                  mr = meanIndex(r,tGLCM);
                  mc = meanIndex(c,tGLCM);
                  m=1; % meanIndex calculated (no need of 2 flags, due to alphabetic order of stats)
          end
      end
    ...
    switch name
        ...
        case 'Correlation'
            stats.(name)(p) = calculateCorrelation(tGLCM,r,c,mr,mc);
        ...
        case 'InvDiffMoment'
            stats.(name)(p) = calculateInvDiffMoment(tGLCM,r,c);

        case 'ClusterShade'
            stats.(name)(p) = calculateClusterShade(tGLCM,term1,term2);

        case 'ClusterProminence'
            stats.(name)(p) = calculateClusterProminence(tGLCM,term2);

        case 'MaxProbability'
            stats.(name)(p) = calculateMaxProbability(tGLCM);
            
        case 'Autocorrelation'
            stats.(name)(p) = calculateAutocorrelation(tGLCM,r,c);
            
        case 'Dissimilarity'
            stats.(name)(p) = calculateDissimilarity(tGLCM,r,c);
        
        case 'Variance'
            stats.(name)(p) = calculateVariance(tGLCM,r);
    end
...
term1= abs(r-c);
term= term1 .* term1 .* glcm(:);
C = sum(term);

%-----------------------------------------------------------------------------
function CS = calculateClusterShade(glcm,term1,term2)
% References: 
% Soh LK, Tsatsoulis C. "Texture Analysis of SAR Sea Ice Imagery Using Gray
% Level Co-Occurrence Matrices" IEEE Trans GRS Vol.37 No.2 pp. 780-795, 1999 

% Calculate the mean of a pixel value in the row and column direction 
%mr = meanIndex(r,glcm);
%mc = meanIndex(c,glcm);

%term1=(r + c - mr - mc);
%term2=term1 .* term1; 
term = term2 .* term1 .* glcm(:);
CS=sum(term);

%term1 = (r + c - mr - mc).^3 .* glcm(:);
% CS=sum(term1);

%-----------------------------------------------------------------------------
function CP = calculateClusterProminence(glcm,term2)
% References: 
% Soh LK, Tsatsoulis C. "Texture Analysis of SAR Sea Ice Imagery Using Gray
% Level Co-Occurrence Matrices" IEEE Trans GRS Vol.37 No.2 pp. 780-795, 1999 
...
%term1 = (r + c - mr - mc);
%term2 = term1 .* term1;
term = term2 .* term2 .* glcm(:);
CP=sum(term);

%term1 = (r + c - mr - mc).^4 .* glcm(:);
%CP=sum(term1);

%-----------------------------------------------------------------------------
function Corr = calculateCorrelation(glcm,r,c,mr,mc)
% References: 
...
% Reference: Haralick RM, Shanmugam K, Dinstein I. "Textural Features for
% Image Classification" IEEE Trans SMC Vol.3 No.6 pp. 610-621, 1973
...
%-----------------------------------------------------------------------------
function I = calculateInvDiffMoment(glcm,r,c)
% Reference: Haralick RM, Shanmugam K, Dinstein I. "Textural Features for
% Image Classification" IEEE Trans SMC Vol.3 No.6 pp. 610-621, 1973
...
% Reference: Soh LK, Tsatsoulis C. "Texture Analysis of SAR Sea Ice Imagery Using Gray
% Level Co-Occurrence Matrices" IEEE Trans GRS Vol.37 No.2 pp. 780-795, 1999 

P=max(glcm(:));

%-----------------------------------------------------------------------------
function A = calculateAutocorrelation(glcm,r,c)
% Reference: Soh LK, Tsatsoulis C. "Texture Analysis of SAR Sea Ice Imagery Using Gray
% Level Co-Occurrence Matrices" IEEE Trans GRS Vol.37 No.2 pp. 780-795, 1999 
...
%-----------------------------------------------------------------------------
function D = calculateDissimilarity(glcm,r,c)
% Reference: Soh LK, Tsatsoulis C. "Texture Analysis of SAR Sea Ice Imagery Using Gray
% Level Co-Occurrence Matrices" IEEE Trans GRS Vol.37 No.2 pp. 780-795, 1999 
...
%-----------------------------------------------------------------------------
function V = calculateVariance(glcm,r)
% Reference: Haralick RM, Shapiro LG. Computer and Robot Vision: Vol. 1,
% Addison-Wesley, 1992, p. 460.  
  
% m=mean2Index(glcm); % m(r)
m=mean(glcm(:));
term1 = (r - m).^2; 
term = term1 .* glcm(:);
V = sum(term);
