function [result] = append_fishShapeContext(fish_candidate, fish_templates)

reshape_size = [100, 100];
nsamp=100;

V1 = double(fish_candidate);
V2 = double(fish_templates);
...
%%% image loading
%%%
%V1=reshape(fish_candidate,reshape_size,reshape_size)';
V1=imresize(V1,reshape_size,'bil');
%V2=reshape(fish_templates,reshape_size,reshape_size)';
V2=imresize(V2,reshape_size,'bil');
[N1,N2]=size(V1);
...
%%% edge detection
%%%
[x2,y2,t2]=append_sc_bdry_extract_3(V2);
nsamp2=length(x2);
if nsamp2>=nsamp
   [x2,y2,t2]=append_sc_get_samples_1(x2,y2,t2,nsamp);
else
...
[x1,y1,t1]=append_sc_bdry_extract_3(V1);

nsamp1=length(x1);
if nsamp1>=nsamp
   [x1,y1,t1]=append_sc_get_samples_1(x1,y1,t1,nsamp);
else
...
while s
   ...
   [BH1,mean_dist_1]=append_sc_sc_compute(Xk',zeros(1,nsamp),mean_dist_global,nbins_theta,nbins_r,r_inner,r_outer,out_vec_1);
   disp('done.')
   % apply the scale estimate from the warped model to the test shape
   disp('computing shape contexts for target...')
   [BH2,mean_dist_2]=append_sc_sc_compute(Y',zeros(1,nsamp),mean_dist_global,nbins_theta,nbins_r,r_inner,r_outer,out_vec_2);
   disp('done.')
   ...
   costmat_shape=append_sc_hist_cost_2(BH1,BH2);
...
%   costmat_theta=abs(atan2(sin(theta_diff),cos(theta_diff)))/pi;
   ...
   cvec=append_sc_hungarian(costmat2);
...
%	 set(h,'linewidth',1)
   ...
   [cx,cy,E]=append_sc_bookstein(X3b,Y3,beta_k);

   % calculate affine cost
   A=[cx(n_good+2:n_good+3,:) cy(n_good+2:n_good+3,:)];
   s=svd(A);
   [svd_u, svd_s, svd_v]=svd(A);
   ...
   d2=max(append_sc_dist2(X3b,X),0);
   U=d2.*log(d2+eps);
   ...
   d2=max(append_sc_dist2(X3b,Xtan),0);
   U=d2.*log(d2+eps);
   fx_wrp=cx(1:n_good)'*U;
   fx=fx_aff+fx_wrp;
...
ssd_all=sqrt(append_sc_dist2(win_list_1,win_list_2));
if 0
...
result.score_aff_cost = aff_cost;
result.score_sc_cost = sc_cost;
result.score_ssd_global = ssd_global;

result.affine_matrix = A;
result.affine_rotate = (cart2pol(svd_u(1,2), svd_u(1,1))- cart2pol(svd_v(1,2), svd_v(1,1)))/pi*180;
end
